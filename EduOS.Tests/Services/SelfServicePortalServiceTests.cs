...
public class SelfServicePortalServiceTests
{
    ...
    public async Task Transport_returns_only_assignment_for_directly_linked_student_in_current_tenant()
    {
        ...
        context.StudentTransports.AddRange(Assignment(10, own.Id, ownVehicle.Id, ownRoute.Id, "Gate A"), Assignment(10, other.Id, otherVehicle.Id, otherRoute.Id, "Gate B"), Assignment(20, foreign.Id, foreignVehicle.Id, foreignRoute.Id, "Gate C")); await context.SaveChangesAsync(); SetTenant(accessor, 10);
    ...
    private static SelfServicePortalService CreateService(EduOSDbContext context, ICurrentUserService currentUser) => new(
        ...
        new GenericRepository<StudentTransport>(context), new GenericRepository<Homework>(context), new GenericRepository<Assignment>(context), new GenericRepository<CourseEnrollment>(context), currentUser, NullLogger<SelfServicePortalService>.Instance);
