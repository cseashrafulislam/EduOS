...
Authorized managers can also create time slots, assign an active instructor only to a subject in the batch's active curriculum, create or deactivate routine entries, and read batch/teacher timetables. Teacher, batch and room overlap checks run inside a serializable retry-aware transaction; room campus/capacity, academic year/term, tenant and teacher ownership are validated server-side. Students and linked guardians can read the timetable for the learner's latest active legacy enrolment through a tenant-scoped, module-entitled, no-store portal endpoint. Canonical batch-to-student placement, substitution, lesson-plan and student elective-registration workflows remain incomplete, so the overall academic area remains Foundation rather than fully implemented.
