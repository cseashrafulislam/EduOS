...
public sealed class FeeBillingController : ControllerBase
{
    ...
    public async Task<IActionResult> SaveStructure([FromBody] SaveFeeStructureDto request, CancellationToken cancellationToken) { if (!ModelState.IsValid) return ValidationProblem(ModelState); var result = await _service.SaveFeeStructureAsync(request, cancellationToken); return StatusCode(result.StatusCode, result); }
    ...
    [Authorize(Roles = "TenantAdmin,Principal,Accountant")]
    public async Task<IActionResult> SetFine([FromBody] SetInvoiceFineDto request, CancellationToken cancellationToken) { if (!ModelState.IsValid) return ValidationProblem(ModelState); var result = await _service.SetFineAsync(request, cancellationToken); return StatusCode(result.StatusCode, result); }
