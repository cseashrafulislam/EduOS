...
public sealed class EmployeeSelfServiceService : IEmployeeSelfServiceService
{
    ...
    public async Task<ApiResponse<EmployeePortalProfileDto>> GetProfileAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var profile = await _employees.GetQueryable().AsNoTracking()
                ...
                .Select(x => new EmployeePortalProfileDto { Reference = x.PublicId, EmployeeCode = x.EmployeeCode, FullName = x.FullName, Phone = x.Phone, Email = x.Email, DesignationId = x.DesignationId, DepartmentId = x.DepartmentId, JoiningDate = x.JoiningDate, Salary = x.Salary, PhotoUrl = x.PhotoUrl, Qualification = x.Qualification, Experience = x.Experience, IsTeacher = x.IsTeacher })
    ...
    public async Task<ApiResponse<EmployeePortalLeaveDto>> ApplyLeaveAsync(EmployeePortalLeaveApplyDto request, CancellationToken cancellationToken = default)
    {
        ...
        if (totalDays > 366) return ApiResponse<EmployeePortalLeaveDto>.ErrorResponse("Leave period cannot exceed 366 days.", 400);
        try
        {
            var hasActiveEmployee = await _employees.GetQueryable().AsNoTracking().AnyAsync(x => x.TenantId == _currentUser.TenantId && x.UserId == _currentUser.UserId && x.IsActive, cancellationToken);
            ...
            if (leaveType.MaxDaysPerYear > 0 && totalDays > leaveType.MaxDaysPerYear) return ApiResponse<EmployeePortalLeaveDto>.ErrorResponse("Requested leave exceeds the leave type annual limit.", 400);
