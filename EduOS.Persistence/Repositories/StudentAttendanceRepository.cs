...
namespace EduOS.Persistence.Repositories
{
    public class StudentAttendanceRepository : GenericRepository<StudentAttendance>, IStudentAttendanceRepository
    {
        ...
        public async Task<List<StudentAttendance>> GetByDateAsync(DateTime date, long classId, long sectionId) => await _dbSet.Include(a => a.Student).Where(a => a.Date.Date == date.Date && a.ClassId == classId && a.SectionId == sectionId).OrderBy(a => a.Student!.Roll).ToListAsync();
