...
- Annual promotion/repeat plus manager-controlled transfer/completion/dropout finalization; final exit is dues-aware, retry-safe and atomically closes legacy and canonical current enrolments.
...
- Exam mark/result workflow: assignment-owned teacher rosters, class-teacher result visibility, manager-controlled deterministic generation and atomic/idempotent publication with published-result immutability.
